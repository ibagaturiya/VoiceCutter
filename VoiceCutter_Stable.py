...
def main():
    ...
    fps = parse_fps(original.GetSetting("timelineFrameRate"), 24.0)
    clips = collect_clips(original, fps)
    ffmpeg = locate_ffmpeg()

    for clip in clips:
        silence = analyze(ffmpeg, clip, threshold, minimum_silence)
        clip["voice"] = voice_from_silence(silence, clip["duration"], padding)

    timeline = original.DuplicateTimeline(
        unique_timeline_name(project, original.GetName() + " - VoiceCutter")
    )
    project.SetCurrentTimeline(timeline)

    subtype = str(timeline.GetTrackSubType("audio", 1)).lower()

    timeline.AddTrack("audio", subtype)
    a_voice = int(timeline.GetTrackCount("audio"))

    timeline.AddTrack("audio", subtype)
    a_nonvoice = int(timeline.GetTrackCount("audio"))

    timeline.AddTrack("video")
    v_voice = int(timeline.GetTrackCount("video"))

    timeline.AddTrack("video")
    v_nonvoice = int(timeline.GetTrackCount("video"))

    timeline.SetTrackName("audio", a_voice, "VOICE")
    timeline.SetTrackName("video", v_voice, "VOICE")
    timeline.SetTrackName("audio", a_nonvoice, "NON-VOICE")
    timeline.SetTrackName("video", v_nonvoice, "NON-VOICE")
    ...
    for clip in clips:
        for interval in clip["voice"]:
            record_bounds = quantize_interval(interval, clip["duration"], clip["record_span"])
            source_bounds = quantize_interval(interval, clip["duration"], clip["source_span"])
    ...
    link_matching(timeline, v_voice, a_voice)

    # 2. NON-VOICE = actual gaps left by VOICE
    voice_items = items_on(timeline, "video", v_voice)
    ...
    link_matching(timeline, 1, 1)
    link_matching(timeline, v_voice, a_voice)
    link_matching(timeline, v_nonvoice, a_nonvoice)
